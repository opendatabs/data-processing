...
fread("data/nbcn-daily_BAS_previous.csv", sep = ";", colClasses = c("character", "Date", rep("numeric", 10))) %>%
  mutate(
  ...
  bind_rows(
    ...
    fread("data/nbcn-daily_BAS_current.csv", sep = ";", colClasses = c("character", "Date", rep("numeric", 10))) %>%
      mutate(
        timestamp = as.POSIXct(
          paste0(
            substr(date, 1, 4),
...
## ----fig.cap="Lufttemperatur seit 1864."--------------------------------------------------------------------------------
meteo %>%
...
httr::GET("https://data.bs.ch/explore/dataset/100233/download/?format=csv&timezone=Europe%2FBerlin", use_proxy(paste0(pw[system=="internet", login], ":", pw[system=="internet", password], "@proxy1.bs.ch"), 3128))  %>%
  content(., "text") %>%
...
## ----fig.cap="Täglicher Stromverbrauch seit 2012."----------------------------------------------------------------------
...
httr::GET("https://data.bs.ch/explore/dataset/100074/download/?format=csv&timezone=Europe%2FBerlin", use_proxy(paste0(pw[system=="internet", login], ":", pw[system=="internet", password], "@proxy1.bs.ch"), 3128)) %>%
  content(., "text") %>%
  fread(sep=";") %>%
...
write.csv2(Pred_lm %>%
           ...
           "data/export/100245_Strom_Wetter.csv", row.names=F, na = "") 
...
## ----fig.cap="Effektiver und erwarteter täglicher Stromverbrauch."------------------------------------------------------
