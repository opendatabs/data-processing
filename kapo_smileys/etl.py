...
def parse_single_messdaten_folder(curr_dir, folder, df_einsatz_days, df_einsatze):
    ...
    for phase in [['Vormessung'], ['Betrieb'], ['Nachmessung'], ['Vormessung', 'Betrieb', 'Nachmessung']]:
        ...
        stat_pro_standort = {
            ...
            'dtv': dtv,
            'link_einzelmessungen': f'https://data.bs.ch/explore/dataset/100268/table/?refine.id_standort={id_standort}&refine.zyklus={zyklus}' if len(phase) > 1 else f'https://data.bs.ch/explore/dataset/100268/table/?refine.id_standort={id_standort}&refine.zyklus={zyklus}&refine.phase={phase}'
        }
