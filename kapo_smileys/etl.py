...
def parse_single_messdaten_folder(curr_dir, folder, df_einsatz_days, df_einsatze):
    ...
    for f in tagesdaten_files:
        ...
        # Assumption: Does not switch to DST. Have to ask, but until than, add one hour if DST
        df['is_dst'] = df['Messung_Timestamp'].apply(lambda x: is_dst(x, pytz.timezone('Europe/Zurich')))
        df.loc[df['is_dst'],'Messung_Timestamp'] =  df['Messung_Timestamp'] + pd.Timedelta(hours=1)
        ...
        df_m = df_m.drop(columns=['id_Standort', 'is_dst'])
...
def parse_einsatzplaene(curr_dir):
    ...
    for f in einsatzplan_files:
        ...
        for col in ['Start_Vormessung', 'Start_Betrieb', 'Start_Nachmessung', 'Ende']:
            ...
            df[col] = df[col].dt.tz_localize('Europe/Zurich', ambiguous='infer', nonexistent='NaT')
