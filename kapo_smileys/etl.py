...
def parse_single_messdaten_folder(curr_dir, folder, df_einsatz_days, df_einsatze):
    ...
    for f in tagesdaten_files:
        ...
        df_m = pd.merge(df_m1, df_einsatze, how='left', left_on=['id_standort', 'Zyklus'], right_on=['id_Standorte', 'Zyklus'])
        df_m = df_m.drop(columns=['id_Standorte'])
...
def main():
    ...
    df_einsatz_days = pd.concat([pd.DataFrame({'id_standort': row.id_Standorte, 'Zyklus': row.Zyklus, 'datum_aktiv': pd.date_range(row.Start_Vormessung, row.Ende, freq='D', normalize=True)})  # , 'Start_Vormessung': row.Start_Vormessung, 'Start_Betrieb': row.Start_Betrieb, 'Start_Nachmessung': row.Start_Nachmessung, 'Ende': row.Ende})
