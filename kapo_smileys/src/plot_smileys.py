...
def create_histogram_plot(curr_dir, id_standort, street_data, phase_order, phase_colors, street_name, zyklus, geschw):
    ...
    for phase in phase_order:
        ...
        plt.bar(base_bins[:-1] + offset, counts_einfahrt, width=bar_width,  label=f'{phase} (Einfahrt)',
                alpha=0.5, color=phase_colors[phase], edgecolor='black', align='edge')
        offset += bar_width
        ...
        plt.bar(base_bins[:-1] + offset, counts_ausfahrt, width=bar_width, label=f'{phase} (Ausfahrt)',
                alpha=0.5, color=phase_colors[phase], edgecolor='black', align='edge', hatch='//')
        offset += bar_width
    ...
    plt.grid(True, which='both', axis='y', linestyle='-', linewidth=0.5, color='grey', alpha=0.7)
