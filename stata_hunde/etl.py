...
def main():
    ...
    df = pd.read_csv(os.path.join("data", "Hunde für OGD.csv"), encoding="windows-1252", sep=";")

    logging.info("Extracting jahr, gemeinde_name, hund_geschlecht, hund_geburtsjahr, hund_alter, hund_rasse, hund_farbe...")
    df_hunde = df[
        [
            "jahr",
            "gemeinde_name",
    ...
    df_hunde = df_hunde.sort_values(
        by=["jahr", "gemeinde_name", "hund_geschlecht", "hund_geburtsjahr", "hund_rasse", "hund_farbe"]
    ...
    df_webtabelle = pd.read_excel(os.path.join("data", "t16-2-03_Webtabelle_Hunde_seit_1970.xlsx"), sheet_name="Gemeinde", skiprows=7, usecols="B, D:F")
    ...
    df_hundebestand = df.groupby(["jahr", "wohnviertel_aktuell", "gemeinde_name"]).size().reset_index(name="anzahl_hunde")
    ...
    df_hundehalter = pd.read_excel(os.path.join("data", "Hundebestandsliste seit 1992.xls"), engine="xlrd", skiprows=5)
    ...
    df_hundehalter = df_hundehalter.rename(columns={"Total Hundehalter (neu erhoben ab 2018)": "anzahl_hundehalter"})[['anzahl_hundehalter']]
