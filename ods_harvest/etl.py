...
def wait_for_idle(harvester_id):
    while True:
        ...
        resp = common.requests_get(
            url=f"https://data.bs.ch/api/automation/v1.0/harvesters/{harvester_id}/",
...
for harv_id in ods_harvester_ids:
    ...
    response = common.requests_post(
        f"https://data.bs.ch/api/automation/v1.0/harvesters/{harv_id}/start/",
        headers={"Authorization": f"apikey {ODS_API_KEY}"},
    )
    ...
    response = common.requests_post(
        f"https://data.bs.ch/api/automation/v1.0/harvesters/{harv_id}/publish/",
        headers={"Authorization": f"apikey {ODS_API_KEY}"},
    )
    handle_http_errors(response)
    wait_for_idle(harv_id)
