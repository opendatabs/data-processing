    ...
    if ct.has_changed(datafile_with_path, do_update_hash_file=False):
        ...
        if not no_file_copy:
            ...
            for filename in files_to_upload:
                export_filename_with_path = credentials.path_work + filename
                if ct.has_changed(export_filename_with_path, do_update_hash_file=False):
                    common.upload_ftp(export_filename_with_path, credentials.ftp_server, credentials.ftp_user, credentials.ftp_pass, '')
                    ct.update_hash_file(export_filename_with_path)
