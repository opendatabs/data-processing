...
def main():
    ...
    if ct.has_changed(datafile_with_path):
        ...
        generated_datasets = create_truncated_dataset(
            gew_rhein_rues_wasser, generated_datasets
        )
        generated_datasets = create_dataset_for_each_year(
            gew_rhein_rues_wasser, generated_datasets
        )
        ...
        for filename in files_to_upload:
            ...
            if ct.has_changed(file_path):
                ...
                common.upload_ftp(
                    file_path, FTP_SERVER, FTP_USER, FTP_PASS, remote_path
                )
        ...
        if realtime_push_all_years:
            ...
            df_to_push["Probenahmedatum_date"] = df_to_push[
            ...
            gew_rhein_rues_wasser_last["Probenahmedatum_date"] = (
                gew_rhein_rues_wasser_last["Probenahmedatum_date"].astype(str)
            )
            gew_rhein_rues_wasser_new["Probenahmedatum_date"] = (
                gew_rhein_rues_wasser_new["Probenahmedatum_date"].astype(str)
            )
...
def split_into_datasets(data):
    ...
    data["Probenahmedatum_date"] = pd.to_datetime(
        data["Probenahmedatum"], format="%d.%m.%Y", errors="coerce"
    )
    ...
    gew_rhein_rues_fest = data.query(
        'Probenahmestelle == "GEW_RHEIN_RUES" and Probentyp == "FESTSTOFF"'
    )
    gew_rhein_rues_wasser = data.query(
        'Probenahmestelle == "GEW_RHEIN_RUES" and Probentyp == "WASSER"'
    )
    oberflaechengew = data.query(
        'Probentyp == "WASSER" and '
...
def create_truncated_dataset(gew_rhein_rues_wasser, generated_datasets):
    ...
    gew_rhein_rues_wasser_truncated = gew_rhein_rues_wasser[
        gew_rhein_rues_wasser.Probenahmejahr.isin(years)
    ]
