...
def locate_indices_non_gv(hdr, pre_hdrs):
    ...
    idx["unguelt"] = (
        find_in_hdrs("ungültige", [hdr]) or find_in_hdrs("ungueltige", [hdr]) or find_in_hdrs("ungultige", [hdr])
    )
    idx["guelt"] = (
        find_in_hdrs("total gültige", [hdr])
        or find_in_hdrs("total gültig", [hdr])
        or find_in_hdrs("gültige", [hdr])
        or find_in_hdrs("gueltige", [hdr])
    )
...
def locate_indices_gv(pre_hdrs, hdr):
    ...
    def next_after(p):
        return next((q for q in all_pos if q > p), len(hdr))
...
def parse_stimmberechtigte(lines):
    ...
    while i < len(lines):
        ...
        if not any(row):
            break
        if row[0].lower().startswith("stimmbeteiligung"):
            break
...
def parse_kennzahlen_block(lines):
    ...
    for i, ln in enumerate(lines):
        ...
        if (
            ("durchschnitt" in joined and "stimmbeteilig" in joined)
            or ("brieflich" in joined)
            or ("elektron" in joined)
        ):
            hdr, hdr_idx = toks, i
            break
    if hdr is None:
        return pd.DataFrame(
            columns=[
                "Gemein_Name",
                "Durchschn_Stimmbet_pro_Abst_Art",
                "Durchschn_Briefl_Ant_pro_Abst_Art",
                "Anz_Elektr_pro_Abst_Art",
            ]
        )
    ...
    while i < len(lines):
        ...
        if i_stb is not None and len(toks) > i_stb:
            v = clean_numeric(toks[i_stb])
            v = v / 100.0 if (not pd.isna(v) and v > 1) else v
            vals["Durchschn_Stimmbet_pro_Abst_Art"] = v
        if i_brf is not None and len(toks) > i_brf:
            v = clean_numeric(toks[i_brf])
            v = v / 100.0 if (not pd.isna(v) and v > 1) else v
...
def main():
    ...
    if os.path.exists(WAHLLOKALE_CSV):
        df_wahllok = pd.read_csv(WAHLLOKALE_CSV, encoding="unicode_escape").rename(
            columns={"Wahllok_Name": "Wahllok_name"}
        )
    ...
    for path in files:
        ...
        for sec in sections:
            ...
            for row in data_rows:
                ...
                stimmr = (
                    clean_num(row[indices["stimmr"]])
                    if indices.get("stimmr") is not None and len(row) > indices["stimmr"]
                    else pd.NA
                )
                eingel = (
                    clean_num(row[indices["eingel"]])
                    if indices.get("eingel") is not None and len(row) > indices["eingel"]
                    else pd.NA
                )
                leer = (
                    clean_num(row[indices["leer"]])
                    if indices.get("leer") is not None and len(row) > indices["leer"]
                    else pd.NA
                )
                unguelt = (
                    clean_num(row[indices["unguelt"]])
                    if indices.get("unguelt") is not None and len(row) > indices["unguelt"]
                    else pd.NA
                )
                guelt = (
                    clean_num(row[indices["guelt"]])
                    if indices.get("guelt") is not None and len(row) > indices["guelt"]
                    else pd.NA
                )
                ja = (
                    clean_num(row[indices["ja"]])
                    if indices.get("ja") is not None and len(row) > indices["ja"]
                    else pd.NA
                )
                nein = (
                    clean_num(row[indices["nein"]])
                    if indices.get("nein") is not None and len(row) > indices["nein"]
                    else pd.NA
                )
                ...
                if sec.is_gv:
                    ...
                    def get(ix):
                        return clean_num(row[ix]) if ix is not None and len(row) > ix else pd.NA
                    ...
                    details_row["gege_anteil_ja_Stimmen"] = to_float_ratio(
                        details_row["Gege_Ja_Anz"], nz(details_row["Gege_Ja_Anz"]) + nz(details_row["Gege_Nein_Anz"])
                    )
                    details_row["sti_anteil_init_stimmen"] = to_float_ratio(
                        details_row["Sti_Initiative_Anz"],
                        nz(details_row["Sti_Initiative_Anz"]) + nz(details_row["Sti_Gegenvorschlag_Anz"]),
                    )
                else:
    ...
    for date_key in sorted(per_date_details.keys()):
        ...
        def assign_ids(df):
            ...
            for _, title in nat_titles:
                id_map[("national", title)] = nid
                nid += 1
            cid = nid
            for _, title in kan_titles:
                id_map[("kantonal", title)] = cid
                cid += 1
        ...
        if not df_det.empty:
            df_det["Abst_Titel"] = df_det.apply(
                lambda r: format_title(r["Abst_Titel"], r.get("has_geg", False)), axis=1
            )
            df_det["Abst_ID_Titel"] = df_det["Abst_ID"].astype(str) + ": " + df_det["Abst_Titel"]
        if not df_ken.empty:
            df_ken["Abst_Titel"] = df_ken.apply(
                lambda r: format_title(r["Abst_Titel"], r.get("has_geg", False)), axis=1
            )
        ...
        if df_wahllok is not None and not df_det.empty:
            ...
            df_det["id"] = (
                df_det["Abst_Datum"]
                + "_"
                + df_det["Abst_ID"].astype(int).astype(str).str.zfill(2)
                + "_"
        ...
        if not df_ken.empty:
            ...
            if {"Stimmr_Anz", "Stimmber_Anz"}.issubset(df_ken.columns):
                ...
                if "Durchschn_Stimmbet_pro_Abst_Art" in df_ken.columns:
                    df_ken["Durchschn_Stimmbet_pro_Abst_Art"] = df_ken["Durchschn_Stimmbet_pro_Abst_Art"].fillna(
                        stimmbet
                    )
                else:
            ...
            if not df_det.empty:
                ...
                for gname, rows in brief_map.items():
                    tmp = (
                        ...
                        .groupby("Abst_ID", as_index=False)["Stimmr_Anz"]
                        .sum()
                ...
                df_briefl = (
                    ...
                    if parts
                    else pd.DataFrame(columns=["Abst_ID", "Gemein_Name", "briefl_stimmr"])
                )
                ...
                if not df_briefl.empty:
                    ...
                    if "Durchschn_Briefl_Ant_pro_Abst_Art" in df_ken.columns:
                        df_ken["Durchschn_Briefl_Ant_pro_Abst_Art"] = df_ken[
                            "Durchschn_Briefl_Ant_pro_Abst_Art"
                        ].fillna(brf_ratio)
                    else:
            ...
            if not ele.empty:
                ...
                if "Anz_Elektr_pro_Abst_Art" in df_ken.columns:
                    df_ken.loc[mask_as, "Anz_Elektr_pro_Abst_Art"] = df_ken.loc[
                        mask_as, "Anz_Elektr_pro_Abst_Art"
                    ].fillna(df_ken.loc[mask_as, "Anz_Elektr_pro_Abst_Art_elecalc"])
            ...
            df_ken["id"] = (
                df_ken["Abst_Datum"]
                + "_"
                + df_ken["Abst_ID"].astype(int).astype(str).str.zfill(2)
                + "_"
        ...
        use_det_cols = (
            [c for c in det_cols_base if c in df_det.columns]
        ...
        use_ken_cols = (
            [c for c in ken_cols_base if c in df_ken.columns]
            + [c for c in ken_cols_gv if c in df_ken.columns]
            + ken_cols_tail
        )
