...
def fix_data(data_file, encoding):
    filename_fixed = os.path.join(credentials.path, 'fixed', os.path.basename(data_file))
...
for index, row in df.iterrows():
    ...
    if row['Verzeichnis'] is not None:
        ...
        for file in raw_files:
            ...
            raw_df = pd.read_table(fix_data(data_file=file, encoding=enc), skiprows=6, header=0, encoding=enc, names=['Geschwindigkeit', 'Zeit', 'Datum', 'Richtung ID', 'Fahrzeuglänge'], error_bad_lines=True, warn_bad_lines=True)
            ...
            if not raw_df.empty:
                ...
                filename_current_measure = os.path.join(credentials.path, 'processed', credentials.filename.replace('.csv', f'_{row["ID"]}_{raw_df["Richtung ID"].iloc[0]}.csv'))
