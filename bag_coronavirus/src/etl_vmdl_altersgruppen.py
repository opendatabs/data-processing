...
def get_partial_raw_df(file_path: str, bin_def: dict) -> pd.DataFrame:
    ...
    logging.info(f"Filter by BS and vacc_date < today ({vmdl.today_string()}), and vacc_date between {bin_def['from_date']} and {bin_def['until_date']}...")
    df_bs = sqldf(f'''
        select * 
        from df 
    ...
    logging.info(f'Calculating age groups. Cases where (age < minimal_age) currently must be errors, replacing with "Unbekannt"...')
    ...
    start = df_bs.vacc_day.min()
    earliest_vacc_day = start if not pd.isna(start) else bin_def['from_date']
    logging.info(f'Creating all combinations of days from {earliest_vacc_day} until {latest_vacc_day}...')
    df_all_days = pd.DataFrame(data=pd.date_range(start=earliest_vacc_day, end=latest_vacc_day).astype(str), columns=['vacc_day'])
    df_all_days_indexed = df_all_days.set_index('vacc_day', drop=False)

    logging.info(f'Create empty table of all combinations for long df...')
    ...
    logging.info(f'Creating long table...')
    df_bs_long = sqldf('''
    ...
    logging.info(f'Adding days without vaccinations to long df...')
...
def get_reporting_df(file_path, bins) -> pd.DataFrame:
    ...
    logging.info(f'Calculating cumulative sums...')
    ...
    logging.info(f'Calculating cumulative sums of _only_ first vaccinations...')
    ...
    logging.info(f'Adding explanatory text for vacc_count...')
    ...
    for bin_def in bins:
        ...
        logging.info(f'Joining pop data and calculating percentages for period between {from_date} and {until_date}...')
...
def get_partial_reporting_df(df_bs_long_all, bin_def) -> pd.DataFrame:
    logging.info(f'Calculating age group "Gesamtbevölkerung"...')
    ...
    logging.info('calculating age group "Impfberechtigte Bevölkerung"...')
...
def get_pop_data(bin_def):
    ...
    logging.info(f'Removing "Unbekannt" age group from population dataset...')
    df_pop_age_group = df_pop_age_group.query('age_group != "Unbekannt"')
    logging.info(f'Calculating count of age group "Impfberechtige Bevölkerung"...')
    df_pop_vacc_allowed = pd.DataFrame({'age_group': 'Impfberechtigte Bevölkerung', 'total_pop': df_pop_age_group.total_pop.sum()}, index=[0])
    logging.info(f'Calculating count of age group "Gesamtbevölkerung"...')
    df_pop_total = pd.DataFrame({'age_group': 'Gesamtbevölkerung', 'total_pop': df_pop_2020.anzahl.sum()}, index=[0])
    logging.info(f'Appending totals for "Impfberechtigte Bevölkerung" and "Gesamtbevölkerung" ')
