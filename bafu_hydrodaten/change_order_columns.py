...
# set column order
def set_column_order(files, columns,  local_export_path=os.path.join(pathlib.Path(__file__).parent,  '/data/Rhein/new_archive/2289_pegel_abfluss_'
...
def change_order_columns(old_path, new_path, local_path, file_pattern, river_id, columns):
    local_export_path = os.path.join(pathlib.Path(__file__).parent,  f'data/new_archive/{river_id}/{river_id}_'
                                                              'bis_2023-04-26.csv')
...
for river_id in river_ids:
    ...
    if river_id == '2106':
        columns = ['timestamp', 'pegel', 'abfluss', 'temperatur', 'datum', 'zeit', 'intervall']
    ...
    change_order_columns(old_path=ftp_remote_dir, new_path=ftp_remote_dir
                         , local_path=os.path.join(pathlib.Path(__file__).parent, f'data/backup/{river_id}'), file_pattern=f'{river_id}_pegel*.csv',
                         river_id=river_id, columns=columns)
