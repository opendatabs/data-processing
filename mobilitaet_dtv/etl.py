...
def main():
    if (
        ct.has_changed(RAW_METADATA_FILENAME)
        ...
        logging.info("Data now in memory!")

        logging.info("Calculating Laengenklasse...")
        bins = [-np.inf, 3.5, 8, np.inf]
        # labels =    ['klein', 'mittel', 'gross']
        labels = ["lt_3.5m", "3.5_to_lt_8m", "gte_8m"]
        df_data["Laengenklasse"] = pd.cut(
            df_data["Fahrzeuglänge"],
            bins=bins,
            labels=labels,
            right=False,
            include_lowest=True,
        ...
        df_dtv_lk_pivot = common.collapse_multilevel_column_names(
            df_dtv_laengenklasse.pivot_table(
                index=["Messung-ID", "Richtung ID"], columns=["Laengenklasse"]
            ).reset_index()
        ).rename(columns={"Messung-ID_": "Messung-ID", "Richtung ID_": "Richtung ID"})

        logging.info("Merging all dtv tables...")
        df_dtv = (
            df_dtv_messung.merge(df_dtv_richtung, how="inner")
        ...
        export_filename = os.path.join("data", "dtv.csv")
