...
for index, row in data.iterrows():
    ...
    if row['art'] == 'Vektor':
        ...
        for shpfile in shpfiles:
            ...
            common.upload_ftp(zipfilepath_relative, credentials.ftp_server, credentials.ftp_user, credentials.ftp_pass, 'harvesters/GVA/data')
            ...
            with open(metadata_file, 'r', encoding='cp1252') as json_file:
                ...
                metadata_for_ods.append({
                    'name': len(shpfiles) > 1 if geocat_uid + ':' + shpfilename_noext else geocat_uid,
                    'title': len(shpfiles) > 1 if row['titel'].replace(':', ': ') + ': ' + shpfilename_noext else row['titel'].replace(':', ': '),
...
ods_metadata.to_csv(ods_metadata_filename, index=False)
