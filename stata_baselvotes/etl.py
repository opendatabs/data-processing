...
def parse_position_grosser_rat(text: str) -> dict[str, str | int]:
    ...
    if result["anzahl_nein"] is None and ("Grosses Mehr gegen" in text or "Grosse Mehrheit gegen" in text or "grosse Mehrheit gegen" in text):
...
def add_parsed_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Parolen Weitere Stichfrage Gegenvorschlag"] = parolen_weitere_parsed.apply(lambda x: x["stichfrage_gegenvorschlag"])
    ...
    df["Position des Grossen Rates Stichfrage Gegenvorschlag"] = position_gr_parsed.apply(lambda x: x["stichfrage_gegenvorschlag_ja"])
    df["Position des Grossen Rates Stichfrage Initiative"] = position_gr_parsed.apply(lambda x: x["stichfrage_initiative_ja"])
