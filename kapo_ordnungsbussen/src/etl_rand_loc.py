...
def main():
    ...
    if ct.has_changed(list_path):
        ...
        export_path = os.path.join(credentials.export_path, 'Ordnungsbussen_OGD_rand-loc.csv')
        logging.info(f'Exporting data to {export_path}...')
        df_export = df_export.loc[df_export['randomly_shifted_lat'].notna()]
...
def transform_for_export(df_all):
    ...
    ch1903p = pyproj.CRS("EPSG:2056")  # CH1903+    
    transformer = pyproj.Transformer.from_crs(wgs84, ch1903p, always_xy=True)
    df_export['lon'], df_export['lat'] = transformer.transform(df_export['GPS Länge'].to_list(), df_export['GPS Breite'].to_list())

    logging.info("Randomizing coordinates and getting rid of data we don't want to have published...")
    df_export = df_export.apply(randomize_coordinates, axis=1)
    
    df_export.drop(['lat', 'lon', 'GPS Länge', 'GPS Breite'], axis=1, inplace=True)
