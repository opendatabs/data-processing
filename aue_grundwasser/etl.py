...
def process(file, x_coords_1416):
    ...
    for sensornr_filter in [10, 20]:
        ...
        if export_stats:
            ...
            df_stat['stat_start_timestamp'] = pd.to_datetime(df_stat.startStatist, format='mixed', dayfirst=True).dt.tz_localize(ZoneInfo('Etc/GMT-1')).dt.tz_convert('UTC')
            df_stat['stat_end_timestamp'] = pd.to_datetime(df_stat.endStatist, format='mixed', dayfirst=True).dt.tz_localize(ZoneInfo('Etc/GMT-1')).dt.tz_convert('UTC')
