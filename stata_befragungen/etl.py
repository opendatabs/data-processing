...
def main():
    datasets = [
        {
            "data_file": os.path.join(
                "data_orig", "55plus/2011-2023/datensatz_55plus_2011_2023_OGD_TEXT.csv"
            ),
            "var_file": os.path.join(
                "data_orig", "55plus/2011-2023/Variablen_55plus_2011_2023_OGD.csv"
            ),
        ...
        {
            "data_file": os.path.join(
                "data_orig", "55plus/2023/datensatz_55plus_2023_OGD_TEXT.csv"
            ),
            "var_file": os.path.join(
                "data_orig", "55plus/2023/Variablen_55plus_2023_OGD.csv"
            ),
        ...
        {
            "data_file": os.path.join(
                "data_orig", "55plus/2019/DATENSATZ2019_OGD_TEXT.csv"
            ),
        ...
        {
            "data_file": os.path.join(
                "data_orig", "55plus/2015/DATENSATZ_2015_TEXT.csv"
            ),
        ...
        {
            "data_file": os.path.join(
                "data_orig", "55plus/2011/WORK_DATENSATZ_2011_TEXT.csv"
            ),
    ...
    for ds in datasets:
        ...
        publish_zip_from_folder(
            os.path.join("data", ds["export_folder"]), ds["ftp_folder"]
        )


def publish_zip_from_folder(dir_to_zip, ftp_folder):
    logging.info(
        f'Creating zip archive of data in folder "{dir_to_zip}" for the web...'
    )
...
def process_single_file(
    ...
    df_data = pd.read_csv(
        io.StringIO(clean_text), encoding=enc, sep=";", engine="python"
    )
    df_data.to_csv(
        os.path.join(data_path, export_folder, f"Daten_{export_file}"), index=False
    )
    if jahr:
        ...
        logging.info(
            "Jahresübergreifend: Filter out questions which just appear in one year..."
        )
    ...
    df_var.to_csv(
        os.path.join(data_path, export_folder, f"Variablen_{export_file}"), index=False
    )
    df_merge = df_data_long.merge(
        df_var, how="left", left_on="variable", right_on="FrageName"
    )
