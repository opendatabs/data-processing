...
def main():
    ...
    r = requests.request('GET', url=f'{credentials.charges_url}?from=2022-01-01T00%3A00%3A00%2B02%3A00', headers=headers)
...
def authenticate():
    ...
    r = requests.request('POST', credentials.auth_url, json=payload, headers=headers)
    return r.json()['token']
