...
EXCEL_PATH = os.getenv("EXCEL_PATH_BAUPUBLIKATIONEN_FOR_MAIL")
...
def main():
    ...
    df["secondaryTenantsTenantName"] = (
        df.loc[df["secondaryTenantsTenantCode"].notna(), "secondaryTenantsTenantCode"]
        .str.split(",")
    ...
    path_export = os.path.join("data", "export", "100352_kantonsblatt.csv")
...
def remove_entries(df):
        ...
        df_new_values = pd.DataFrame(
            {
                ...
                "checked_by": "",
                "double_checked_by": "",
                "remove": [True] * len(new_values[sheet]),
    ...
    if len(new_values["registrationOfficeDisplayName"]) + len(new_values["onBehalfOf"]) > 0:
        ...
        msg = common.email_message(
            subject="Data removed from Kantonsblatt (100352). Please check if person data.",
            text=text,
            img=None,
            attachment=None,
        )
