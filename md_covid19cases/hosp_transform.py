...
df0['Total'] = df0.sum(axis=1, skipna=True, numeric_only=True)

df1 = parse_data_file(1)
df1['Total_non_resident'] = df1[credentials.hosp_df1_total_non_resident_columns].sum(axis=1, skipna=True, numeric_only=True)
df1['Total_resident'] = df1[credentials.hosp_df1_total_resident_columns]

df2 = parse_data_file(2)
df2['Total_ips'] = df2[credentials.hosp_df2_total_ips_columns].sum(axis=1, skipna=True, numeric_only=True)
...
df_merged['Datum'] = pd.to_datetime(df_merged['Datum'], format='%d-%m-%Y', errors='coerce')
print(f'Filtering columns...')
df_public = df_merged[['Datum', 'Total', 'Total_non_resident', 'Total_resident', 'Total_ips']]
