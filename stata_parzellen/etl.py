...
def main():
    ...
    if ct.has_changed(parzellen_data_file):
        logging.info("Reading data from 4 datasets...")
        ...
        df_wohnviertel = common.pandas_read_csv(
            ...
            sep=";",
            dtype={"wov_id": "str"},
        )[["wov_id", "wov_label", "wov_name", "gemeinde_name"]]
        ...
        df_export = (
            df.merge(df_wohnviertel, left_on="WOV_ID", right_on="wov_id", how="left")
            .merge(df_bezirk, left_on="BEZ_ID", right_on="bez_id", how="left")
            .merge(df_block, left_on="BLO_ID", right_on="blo_id", how="left")
            .drop(columns=["wov_id"])
        )
        logging.info("Exporting, uploading and updating data in ODS...")
