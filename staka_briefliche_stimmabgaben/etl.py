...
def main():
    ...
    logging.info(
        "check if data from latest Urnengang is already in the df, if not add it"
    )
    if date_urnengang not in dates:
        logging.info(f"Add data of currently active Urnengang of {date_urnengang}")
        df_latest = make_df_for_publ(
            latest_file=latest_file, datetime_urnengang=datetime_urnengang
        )
    ...
    df_wahlen = pd.read_csv(
        os.path.join("data/Basel_briefliche_stimmabgaben/Termine/wahlen.csv")
    )
    df_abst = pd.read_csv(
        os.path.join("data/Basel_briefliche_stimmabgaben/Termine/abstimmungen.csv")
    )
    df_publ["abstimmungen"] = [
        "Ja" if date in df_abst["Abstimmungstermin"].values else "Nein"
        for date in df_publ["datum_urnengang"]
    ]
    df_publ["wahlen"] = [
        "Ja" if date in df_wahlen["Datum"].values else "Nein"
        for date in df_publ["datum_urnengang"]
    ]
    df_publ["wahlen_typ"] = [
        df_wahlen.loc[df_wahlen["Datum"] == row["datum_urnengang"], "Typ"].item()
        if row["wahlen"] == "Ja"
        else ""
    ...
    dates_not_listed = df_publ[df_publ["abstimmungen"] == "Nein"][
        df_publ["wahlen"] == "Nein"
    ]["datum_urnengang"]
    dates_not_listed = list(dates_not_listed.unique())
    df_publ = df_publ[
        (df_publ["abstimmungen"] != "Nein") | (df_publ["wahlen"] != "Nein")
    ]
    if dates_not_listed != []:
...
def get_data_2020():
    ...
    file_list = glob.glob(
        os.path.join("data/Basel_briefliche_stimmabgaben/2020", pattern)
    )
    df_all = pd.DataFrame()
...
def get_previous_data_from_20210307():
    ...
    file_list = glob.glob(
        os.path.join("data", "Basel_briefliche_stimmabgaben", pattern)
    )
    df_all = pd.DataFrame()
    for file in file_list:
...
def make_df_for_publ(latest_file, datetime_urnengang):
    ...
    df_stimmabgaben = pd.read_excel(
        latest_file, sheet_name=0, header=None, names=columns, skiprows=6
    )
