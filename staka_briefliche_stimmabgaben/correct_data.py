...
# obtain all entries with the wrong date in 100224
req = common.requests_get(f'https://data.bs.ch/api/v2/catalog/datasets/100224/exports/json?refine=abstimmungsdatum:{datum}&limit=-1&offset=0&timezone=UTC', auth=(credentials.username, credentials.password))
file = req.json()
df = pd.DataFrame.from_dict(file)


# remove the records with the wrong date in 100224
payload = df.to_json(orient="records")
