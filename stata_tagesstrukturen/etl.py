...
def main():
    ...
    process_anzahl_plaetze(df_schulexterne_nach_gemeinde,
                           df_schuleigene_nach_gemeinde,
                           df_tagesferien)

    process_oeffentliche_ausgaben()

    process_anzahl_kinder(df_schulexterne_nach_gemeinde,
                          df_schuleigene_nach_gemeinde,
                          df_tagesferien)
...
def process_anzahl_plaetze(df_schulexterne, df_schuleigene, df_tagesferien):
    ...
    for y in years:
        ...
        if not tf_row.empty:
            _assert_close("tagesferien (Plätze vs Tagesferien-Wochen)",
...
def process_anzahl_kinder(df_schulexterne, df_schuleigene, df_tagesferien):
    ...
    for y in years:
        ...
        tf_row = tf_agg.loc[tf_agg["jahr"] == y]
        if not tf_row.empty:
            _assert_close("tagesferien (Kinder vs Teilnehmende)",
                          _cell0(row, "tagesferien"),
                          float(tf_row["teilnehmende"].iloc[0] or 0),
                          y)
            _assert_close("ferienbetreuung (Kinder vs Buchungen)",
