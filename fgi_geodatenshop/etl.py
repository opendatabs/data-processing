...
def extract_layers(
    layer_element, namespaces, data, name_hierarchy=None, title_hierarchy=None
):
    ...
    if layer_name is not None and layer_title is not None:
        ...
        current_name_hierarchy = (
            f"{name_hierarchy}/{layer_name}" if name_hierarchy else layer_name
        )
        current_title_hierarchy = (
            f"{title_hierarchy}/{layer_title}" if title_hierarchy else layer_title
        )
...
def process_wms_data(url_wms):
    ...
    if capability_layer is not None:
        ...
        df_wms = pd.DataFrame(
            data, columns=["Name", "Layer", "Hier_Name", "Hier_Titel"]
        )
    return df_wms
...
def extract_meta_geocat(geocat_uid):
    ...
    geocat_url = (
        f"https://www.geocat.ch/geonetwork/srv/api/records/{geocat_uid}/formatters/xml"
    )
    ...
    if response.status_code == 200:
        ...
        logging.warning(
            f"Failed to fetch data from {geocat_url}: Status {response.status_code}"
        )
    xml_data = response.content
...
def save_geodata_for_layers(wfs, df_fgi, file_path):
    ...
    for index, row in meta_data.iterrows():
        if row["import"]:
            ...
            common.upload_ftp(
                geopackage_file, FTP_SERVER, FTP_USER, FTP_PASS, ftp_remote_dir
            )
            ...
            metadata_for_ods.append(
                {
                    ...
                    "references": "; ".join(
                        filter(
                            None, [row["mapbs_link"], row["geocat"], row["referenz"]]
                        )
                    ),
                    "theme": str(row["theme"]),
                    ...
                    "source_dataset": "https://data-bs.ch/opendatasoft/harvesters/GVA/data/"
                    + file_name,
    ...
    if len(metadata_for_ods) > 0:
        ...
        ods_metadata.to_csv(
            ods_metadata_filename, index=False, sep=";", encoding="utf-8"
        )
...
def get_name_col(wfs, df_wfs):
    ...
    for index, row in df_wfs.iterrows():
        ...
        try:
            ...
            logging.info(
                f"Column names of layer '{layer}' successfully saved to '{file_path}'."
            )
    ...
    for index, row in df_fgi.iterrows():
        ...
        for layer in layers:
            try:
                ...
                logging.info(
                    f"layer '{layer}' successfully loaded and number of columns saved."
                )
            except Exception as e:
        ...
        file_path = os.path.join(
            "data", f"schema_files/templates/{file_name}", "Anzahl der Spalten"
        )
...
def main():
    ...
    df_wfs["Gruppe"] = df_wfs.apply(
        lambda row: extract_second_hier_name(row, df_wms), axis=1
    )
